...
const actions = {
  getUserinfo: ({ commit }) => {
    return new Promise(async resolve => {
      ...
      try {
        ...
        commit('SETHOMETAB', data.home_tab)
        commit('SETCHECKREMIND', data.checkRemind)
        console.log(
          `账号身份chatService--${data.chatService}:${
            data.chatService == '0' ? '客户' : '客服'
          }`
        )
        console.log(
          `是否为技术人员isTec--${data.isTec}:${
            data.isTec == '0' ? '非技术人员' : '技术人员'
          }`
        )
        if (data.product.length) {
          if (!getLocalStorage('systemIdx')) {
  ...
  addQuestionList: ({ commit }, list) => {
    return commit('ADDQUESTIONLIST', list)
  },
  setDocumentTitleFlg: ({ commit }, flg) => {
    return commit('SETDOCUMENTTITLEFLG', flg)
  },
  setQuestionRemind: ({ commit }, obj) => {
    return commit('SETQUESTIONREMIND', obj)
  },
  setSupportRemind: ({ commit }, obj) => {
    return commit('SETSUPPORTREMIND', obj)
  },
  setNeedRemind: ({ commit }, obj) => {
    return commit('SETNEEDREMIND', obj)
  },
  setSystemRemind: ({ commit }) => {
    return commit('SETSYSTEMREMIND')
  }
}

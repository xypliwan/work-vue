    ...
    <div class="left">
      <div class="box-title">
        ...
        <i
          ...
          @click="editPlate('0', {})"
          v-if="hasPermissionBtn('first-model-add')"
        ></i>
      </div>
      <div class="list" v-loading="palteLoading">
        <ul>
          <li
            ...
            :key="item.km_id"
            class="item"
            :class="currentIndex == item.km_id ? 'item-active' : ''"
            @click.stop="changeItem(item.km_id)"
          >
            {{ item.title }}
            <span class="operat-btn" v-show="i !== 0">
              <i
                class="iconfont icon-xiugai-landi-"
                @click="editPlate('1', item)"
                v-if="hasPermissionBtn('first-model-edit')"
              ></i>
              <i

...
import { getRemindContent } from '@/api/user'
...
export default {
  ...
  mounted() {},
  computed: {
    ...mapGetters(['productor', 'checkRemind'])
  },
  watch: {
    chatItemId: {
      immediate: true,
  ...
  methods: {
    ...
    async needDetailUpdate() {
      try {
        await needDetailUpdate({
          list_id: this.chatItemId,
          hours: this.workHoursNum
        })
      } catch (error) {
        this._message(error)
      }
